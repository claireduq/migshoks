...
clear
* Ne
import excel "$Data_raw\Claves\Claves.xlsx", sheet("Claves") firstrow
...
drop CVE_MUN
save "$Data_built\Claves2017.dta", replace
...
keep geo* year
duplicates drop
save "$Data_built\Claves_IPUMS_l", replace

use "$Data_built\Claves_IPUMS_l.dta", clear
keep if year==2000
keep geolev2 geo1_mx2000 geo2_mx2000
save $Data_built\cl00.dta, replace

use "$Data_built\Claves_IPUMS_l.dta", clear
keep if year==2005
keep geolev2 geo1_mx2005 geo2_mx2005
save $Data_built\cl05.dta, replace

use "$Data_built\Claves_IPUMS_l.dta", clear
keep if year==2010
keep geolev2 geo1_mx2010 geo2_mx2010
save $Data_built\cl10.dta, replace

use "$Data_built\Claves_IPUMS_l.dta", clear
keep if year==2015
keep geolev2 geo1_mx2015 geo2_mx2015
save $Data_built\cl15.dta, replace

use $Data_built\cl00.dta, clear
*gen firstobs = 2000
bysort geolev2: gen n = _N // number of mun splits since 1960
gen geo2_mx2005=geo2_mx2000
merge 1:m geo2_mx2005 using $Data_built\cl05
...
merge 1:m geo2_mx2010 using $Data_built\cl10
...
merge 1:m geo2_mx2015 using $Data_built\cl15
sort geolev2 geo2_mx2005
...
drop n _merge
su
save $Data_built\Claves_1960_2015.dta, replace

** Add 2017 (here need to do everything manually!)
use $Data_built\Claves_1960_2015.dta, clear
gen geo2_mx2017=geo2_mx2015
merge 1:m geo2_mx2017 using $Data_built\Claves2017
...
save $Data_built\Claves_1960_2017.dta, replace


use $Data_built\Claves_1960_2015.dta, clear
drop geo2_mx2015 geo1_mx2015 //geo2_mx2017 geo1_mx2017
duplicates drop
save $Data_built\Claves_1960_2010.dta, replace

cap erase "$Data_built\cl00.dta"
cap erase "$Data_built\cl05.dta"
cap erase "$Data_built\cl10.dta"
cap erase "$Data_built\cl15.dta"
cap erase "$Data_built\Claves_IPUMS_l.dta"
cap erase "$Data_built\Claves2017.dta"

...
save $Data_built\sec_comm_activation.dta, replace

clear
forvalues x=9/56 {
import delimited $Data_raw\SecureComm\page`x'-page-1-table-1.csv, bindquote(strict) varnames(1) encoding(UTF-8)
keep state county activationdate
append using $Data_built\sec_comm_activation.dta
save $Data_built\sec_comm_activation.dta, replace
sleep 200
clear
}

use $Data_built\sec_comm_activation.dta, clear
...
merge 1:1 county state using $Data_built\sancturay_cities_wide.dta
drop _merge 
...
save $Data_built\sec_comm_activation_wide.dta, replace

replace county=ustrregexra(county, " Census Area", "", 1)
replace county=ustrregexra(county, " Borough", "", 1)
replace county=ustrregexra(county, " Parish", "", 1)
replace county=strupper(county)

save $Data_built\sec_comm_activation_wide_mat.dta, replace

/*
use $Data_built\sec_comm_activation_wide.dta, clear
...
save $Data_built\sec_comm_sanc_long.dta, replace
...
save $Data_built\sec_comm_activation_usav.dta, replace

cap erase "$Data_built\sec_comm_sanc_long.dta"
*/

cap erase "$Data_built\sec_comm_activation.dta"

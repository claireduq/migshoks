...
file open `fh' using git_migshks_code\EMIFlabels\my_labels_`x'.do , write replace
forvalues i = 1/`N' {
...
type git_migshks_code\EMIFlabels\my_labels_`x'.do
}
...
tempname fh
local N = c(N)
file open `fh' using git_migshks_code\EMIFlabels\my_values_2003.do , write append
...
// look at new dofile
type git_migshks_code\EMIFlabels\my_values_2003.do


forvalues x=1999/2012 {
import delimited Data_raw\EMIF\ENORTE_procsur`x'.csv, clear
do git_migshks_code\EMIFlabels\my_labels_`x'.do
tempfile procsur_`x'
save `procsur_`x''
}
...
replace var = strlower(var)
keep if var=="mun_nac" | var=="p13_3_1c" | var=="p13_3_1e" 
rename var var
//rename val val`x'
rename descr descr`x'
tempfile pds_val_`x'
save `pds_val_`x''
}
...
keep if var=="p13_3_1c"
keep if valor>0
drop if valor>=58
rename valor CLAVE_LOC
drop var
...
ta CLAVE_L if _m==1
drop if _m==2
gen obs=1 if _m ==3 // only those that can be matched at city (rather than state) level, drops roughly 1/3 of obs
drop _m
drop desc
...
ta CLAVE_L if _m==1
drop if _m==2
gen obs=1 if _m ==3 // only thise that can be matched at city (rather than state) level, drops roughly 1/3 of obs
drop _m
...
drop desc PMSA MSA LOCALIDADCIUDAD CLAVE_CNTY ESTADO METROPOLIS
save `emif_`x'', replace
}
...
collapse (mean) precision migr_share obs  migwdest migrants (sum) scweight*, by(geo2 geolev y m ym)
...
save "Data_built\EMIF\Shock_EMIF_SecComm_Sanc_2.dta", replace
